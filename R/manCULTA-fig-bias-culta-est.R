...
#' FigBiasCULTAEst(results_culta_est)
#' }
#'
#' @family Figure Functions
#' @keywords manCULTA figure
#' @export
FigBiasCULTAEst <- function(results_culta_est) {
  Parameters <- bias <- Separation <- NULL
  ...
  ggplot2::ggplot(
    ...
    ggplot2::scale_x_continuous(
      breaks = sort(
        unique(results_culta_est$Parameters)
      )
    ) +
    ggplot2::theme_bw() +
    ggplot2::theme(
